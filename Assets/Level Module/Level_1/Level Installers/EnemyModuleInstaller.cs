...
public class EnemyModuleInstaller : MonoInstaller
{
    ...
    {
        Container.BindInterfacesAndSelfTo<IFinder>().FromMethod(
            c => new FreqiencyAroundFinder(new AroundItselfPlayerFinder(5), 1f))
    ...
    {
        Container.BindInterfacesAndSelfTo<MoverToPosition>().FromNew().AsTransient().WhenInjectedInto<RobotBombMovement>();

        //MoverToPosition moveToPosition = Container.Instantiate<MoverToPosition>();
        //moveToPosition.Init(3);
        //Container.BindInterfacesAndSelfTo<IMover>().FromNew()
